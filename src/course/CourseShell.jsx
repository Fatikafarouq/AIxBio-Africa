...
const AdminDashboard=({go})=>{
  ...
  const saveSettings=async()=>{const {error:e}=await supabase.rpc("admin_update_course_settings",{p_is_published:settings.is_published,p_participant_applications_open:settings.participant_applications_open,p_facilitator_applications_open:settings.facilitator_applications_open});if(e){setError(e.message);return;}await load();};
